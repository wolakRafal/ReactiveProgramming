...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs.collect {
      case searchTerm => searchTerm.replace(" ", "_")
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(Success(_)).onErrorReturn(t => Failure(t))
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = obs.debounce(totalSec seconds)
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.flatMap {
